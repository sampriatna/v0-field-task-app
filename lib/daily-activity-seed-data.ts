/** Seed definitions for Daily Activity SOP — ported from nusafood-v2. */
type ReportTemplateCategory = string;
type ReportTemplateKind = "daily_required" | "special_task" | "issue_quick";

export type DailyActivitySeedDef = {
  code: string;
  title: string;
  category: ReportTemplateCategory;
  position_group: string | null;
  outlet_code?: string | null;
  standard_result: string;
  requires_photo: boolean;
  /** Catatan wajib meski status Aman */
  requires_note?: boolean;
  is_required_daily: boolean;
  kind?: ReportTemplateKind;
  target_time_start?: string;
  target_time_end?: string;
  sort_order: number;
  checklist: string[];
};

export const DAILY_ACTIVITY_SEED_TEMPLATES: DailyActivitySeedDef[] = [

  // —— Waiters (5) ——
  {
    code: "RTPL-W01",
    title: "Opening Area Customer",
    category: "Opening",
    position_group: "Waiters",
    standard_result:
      "Area customer siap buka: bersih, meja rapi, lampu/AC/musik OK, siap terima tamu.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "07:30",
    target_time_end: "08:30",
    sort_order: 1,
    checklist: [
      "Lantai area customer bersih / tidak becek",
      "Meja dan kursi rapi, tidak goyang",
      "Peralatan makan/serbet tersedia di stasiun",
      "Lampu dan AC menyala sesuai standar",
      "Musik / ambience aktif",
      "Pintu masuk dan area depan bersih",
      "Tidak ada sampah atau barang berserakan",
    ],
  },
  {
    code: "RTPL-W02",
    title: "Bersihin WC",
    category: "Cleaning",
    position_group: "Waiters",
    standard_result:
      "WC bersih, kering, tidak bau, sabun/tisu tersedia, sampah kosong, siap dipakai customer.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "09:00",
    target_time_end: "10:00",
    sort_order: 2,
    checklist: [
      "Lantai sudah disikat / dipel",
      "Closet bersih",
      "Wastafel bersih",
      "Kaca bersih",
      "Sampah sudah dibuang",
      "Sabun tersedia",
      "Tisu tersedia",
      "Tidak bau",
      "Lantai tidak becek / tidak ada genangan air",
    ],
  },
  {
    code: "RTPL-W03",
    title: "Cek Meja dan Kursi",
    category: "Cleaning",
    position_group: "Waiters",
    standard_result: "Semua meja/kursi bersih, stabil, siap pakai customer.",
    requires_photo: false,
    is_required_daily: true,
    target_time_start: "10:00",
    target_time_end: "11:00",
    sort_order: 3,
    checklist: [
      "Permukaan meja bersih (tidak lengket)",
      "Kursi bersih dan tidak goyang",
      "Tidak ada sisa makanan / noda",
      "Layout meja rapi sesuai standar",
      "Condiment / tissue meja lengkap",
    ],
  },
  {
    code: "RTPL-W04",
    title: "Cek Tanaman",
    category: "Maintenance",
    position_group: "Waiters",
    standard_result: "Tanaman hijau, pot bersih, tidak layu / kering.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "11:00",
    target_time_end: "12:00",
    sort_order: 4,
    checklist: [
      "Daun tidak layu / kuning berlebih",
      "Tanah cukup lembab (disiram jika perlu)",
      "Pot dan area sekitar bersih dari sampah daun",
      "Tanaman tidak menghalangi jalan customer",
    ],
  },
  {
    code: "RTPL-W05",
    title: "Closing Area Customer",
    category: "Closing",
    position_group: "Waiters",
    standard_result: "Area customer bersih, sampah dibuang, siap buka esok hari.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "21:00",
    target_time_end: "22:00",
    sort_order: 5,
    checklist: [
      "Semua meja dibersihkan",
      "Kursi dirapikan",
      "Lantai dipel / disapu",
      "Sampah area customer dibuang",
      "Lampu / AC dimatikan sesuai SOP",
      "Pintu / area aman",
    ],
  },
  // —— Bar (5) ——
  {
    code: "RTPL-B01",
    title: "Opening Bar",
    category: "Opening",
    position_group: "Bar",
    standard_result: "Bar siap operasional: mesin OK, area bersih, stok awal cukup.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "07:30",
    target_time_end: "08:30",
    sort_order: 1,
    checklist: [
      "Area bar bersih dan rapi",
      "Mesin espresso menyala / siap",
      "Grinder dicek dan siap",
      "Steam wand bersih",
      "Stok susu / sirup cukup untuk opening",
      "Cup, lid, sedotan tersedia di stasiun",
    ],
  },
  {
    code: "RTPL-B02",
    title: "Cek Stok Cup / Sedotan",
    category: "Stock",
    position_group: "Bar",
    standard_result: "Stok cup, lid, sedotan cukup untuk operasional hari ini.",
    requires_photo: false,
    is_required_daily: true,
    target_time_start: "09:00",
    target_time_end: "10:00",
    sort_order: 2,
    checklist: [
      "Cup hot cukup",
      "Cup cold cukup",
      "Lid tersedia",
      "Sedotan tersedia",
      "Tissue / napkin stasiun cukup",
      "Catat item yang hampir habis",
    ],
  },
  {
    code: "RTPL-B03",
    title: "Cek Mesin dan Grinder",
    category: "Maintenance",
    position_group: "Bar",
    standard_result: "Mesin dan grinder berfungsi normal, tidak ada kebocoran / bunyi aneh.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "10:00",
    target_time_end: "11:00",
    sort_order: 3,
    checklist: [
      "Tekanan mesin normal",
      "Grinder mengeluarkan bubuk konsisten",
      "Tidak ada kebocoran air / uap",
      "Drip tray bersih",
      "Group head bersih",
    ],
  },
  {
    code: "RTPL-B04",
    title: "Bersihin Bar",
    category: "Cleaning",
    position_group: "Bar",
    standard_result: "Area bar bersih, counter kering, alat dicuci / disimpan rapi.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "14:00",
    target_time_end: "15:00",
    sort_order: 4,
    checklist: [
      "Counter bar dibersihkan",
      "Alat shaker / spoon dicuci",
      "Spill mat / drip tray bersih",
      "Lantai area bar tidak lengket",
      "Sampah bar dibuang",
    ],
  },
  {
    code: "RTPL-B05",
    title: "Closing Bar",
    category: "Closing",
    position_group: "Bar",
    standard_result: "Bar ditutup sesuai SOP: bersih, mesin off, stok diamankan.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "21:00",
    target_time_end: "22:00",
    sort_order: 5,
    checklist: [
      "Backflush / bilas mesin sesuai SOP",
      "Grinder dimatikan / dibersihkan",
      "Susu dan bahan perishable disimpan",
      "Area bar bersih dan kering",
      "Listrik / gas non-esensial dimatikan",
      "Sampah dibuang",
    ],
  },
  // —— Dapur (5) ——
  {
    code: "RTPL-D01",
    title: "Opening Dapur",
    category: "Opening",
    position_group: "Dapur",
    standard_result: "Dapur siap produksi: bersih, peralatan siap, suhu cold storage OK.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "07:00",
    target_time_end: "08:00",
    sort_order: 1,
    checklist: [
      "Area dapur bersih sebelum mulai",
      "Peralatan utama siap pakai",
      "Kompor / oven dicek",
      "Chiller / freezer suhu normal",
      "Bahan opening tersedia di stasiun",
      "Tangan / apron / kebersihan personal OK",
    ],
  },
  {
    code: "RTPL-D02",
    title: "Cek Bahan Siap Jual",
    category: "Stock",
    position_group: "Dapur",
    standard_result: "Bahan siap jual cukup, layak, tidak expired / busuk.",
    requires_photo: false,
    is_required_daily: true,
    target_time_start: "08:00",
    target_time_end: "09:00",
    sort_order: 2,
    checklist: [
      "Stok menu utama cukup untuk service",
      "Tidak ada bahan busuk / bau tidak wajar",
      "Label tanggal / FIFO dipatuhi",
      "Item hampir habis dicatat",
      "Bumbu / sauce stasiun lengkap",
    ],
  },
  {
    code: "RTPL-D03",
    title: "Produksi Bahan Harian",
    category: "Production",
    position_group: "Dapur",
    standard_result: "Produksi harian selesai sesuai target, disimpan dengan benar.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "09:00",
    target_time_end: "11:00",
    sort_order: 3,
    checklist: [
      "Produksi sesuai daftar harian",
      "Porsi / takaran sesuai resep",
      "Hasil disimpan di wadah bersih berlabel",
      "Suhu penyimpanan sesuai standar",
      "Area produksi dibersihkan setelah selesai",
    ],
  },
  {
    code: "RTPL-D04",
    title: "Cek Chiller / Freezer",
    category: "Maintenance",
    position_group: "Dapur",
    standard_result: "Chiller & freezer dingin sesuai standar, rapi, tidak bau.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "12:00",
    target_time_end: "13:00",
    sort_order: 4,
    checklist: [
      "Suhu chiller dalam rentang aman",
      "Suhu freezer dalam rentang aman",
      "Pintu menutup rapat",
      "Tidak ada bau busuk",
      "Barang tersusun rapi / tidak overfill",
      "Tidak ada kebocoran / es berlebih abnormal",
    ],
  },
  {
    code: "RTPL-D05",
    title: "Closing Dapur",
    category: "Closing",
    position_group: "Dapur",
    standard_result: "Dapur bersih, sampah dibuang, api/listrik aman, siap esok hari.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "21:00",
    target_time_end: "22:00",
    sort_order: 5,
    checklist: [
      "Peralatan dicuci dan disimpan",
      "Kompor / oven dimatikan",
      "Area dapur dipel / disapu",
      "Sampah dapur dibuang",
      "Bahan disimpan kembali ke cold storage",
      "Listrik non-esensial dimatikan",
    ],
  },
  // —— PA / OB / Public Area — Kopi Buri Umah (KBU) ——
  {
    code: "RTPL-PA01",
    title: "Opening Public Area Customer",
    category: "Opening",
    position_group: "PA",
    outlet_code: "KBU",
    standard_result:
      "Area customer siap buka, bersih, kering, tidak bau, tidak ada sampah kecil, meja kursi rapi, lantai aman diinjak, dan semua titik terlihat layak untuk tamu.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "08:30",
    target_time_end: "10:00",
    sort_order: 1,
    checklist: [
      "Area depan outlet disapu bersih dari debu, daun, puntung rokok, plastik, tisu, dan sampah kecil",
      "Area masuk / pintu / jalur customer bersih dan tidak licin",
      "Meja customer dilap bersih, tidak lengket, tidak ada noda makanan/minuman",
      "Kursi customer dirapikan sesuai posisi standar",
      "Lantai area makan disapu dan dipel, tidak ada bekas minyak, tanah, atau remah",
      "Kolong meja dicek, tidak ada sampah tersembunyi",
      "Tempat sampah customer dikosongkan jika penuh, plastik diganti bila kotor/bau",
      "Area tunggu / lobby / sekitar kasir luar bersih dan rapi",
      "Kaca / pintu / area terlihat depan dilap jika ada bekas tangan atau noda",
      "Foto laporan menampilkan area luas (bukan close-up satu titik saja)",
    ],
  },
  {
    code: "RTPL-PA02",
    title: "Toilet Customer Check",
    category: "Cleaning",
    position_group: "PA",
    outlet_code: "KBU",
    standard_result:
      "Toilet bersih, tidak bau pesing, lantai kering/aman, kloset bersih, wastafel bersih, sabun/tisu tersedia, dan tidak ada sampah menumpuk.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "09:00",
    target_time_end: "10:00",
    sort_order: 2,
    checklist: [
      "Kloset disikat bersih bagian dalam, dudukan, pinggir, dan area belakang",
      "Lantai toilet disikat/pel, tidak licin, tidak ada rambut, tisu, atau noda kuning",
      "Wastafel dibersihkan dari noda air, sisa sabun, rambut, dan kerak",
      "Kaca toilet dilap, tidak buram dan tidak banyak bercak air",
      "Tempat sampah toilet dikosongkan dan plastik diganti bila kotor/bau",
      "Dinding sekitar kloset dicek, tidak ada cipratan atau noda",
      "Gayung/ember/alat toilet dirapikan dan bersih",
      "Sabun, tisu, atau kebutuhan toilet dicek (habis → tulis kendala)",
      "Bau toilet dicek (masih bau setelah dibersihkan → lapor)",
      "Foto menunjukkan kloset, lantai, wastafel, dan tempat sampah",
    ],
  },
  {
    code: "RTPL-PA03",
    title: "Area Makan Customer Check",
    category: "Cleaning",
    position_group: "PA",
    outlet_code: "KBU",
    standard_result:
      "Area makan customer selalu siap dipakai, meja tidak lengket, kursi rapi, lantai bersih, tidak ada sampah, dan area nyaman untuk tamu.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "10:00",
    target_time_end: "12:00",
    sort_order: 3,
    checklist: [
      "Meja yang sudah dipakai langsung dibersihkan",
      "Sisa piring/gelas kotor dibantu angkat ke area cuci sesuai arahan",
      "Permukaan meja dilap sampai tidak berminyak/lengket",
      "Kursi dikembalikan ke posisi rapi",
      "Lantai sekitar meja disapu dari remah, nasi, tulang, tisu, plastik, dan sampah kecil",
      "Kolong meja dicek setelah customer pergi",
      "Tissue, saus, sendok, atau perlengkapan meja dicek jika area tersebut pakai",
      "Area tidak dibiarkan kotor lebih dari 5 menit setelah customer pergi",
      "Jika area ramai dan belum sempat bersih, wajib tulis kendala (bukan diam)",
      "Foto menampilkan area yang sudah dirapikan (bukan sebelum kerja)",
    ],
  },
  {
    code: "RTPL-PA04",
    title: "Halaman & Parkiran Check",
    category: "Cleaning",
    position_group: "PA",
    outlet_code: "KBU",
    standard_result:
      "Area depan terlihat bersih dari jalan, tidak ada sampah kecil, tidak ada daun menumpuk, parkiran rapi, dan kesan pertama customer bagus.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "10:00",
    target_time_end: "11:30",
    sort_order: 4,
    checklist: [
      "Area parkir disapu dari daun, plastik, puntung rokok, botol, tisu, dan sampah kecil",
      "Jalur masuk customer bersih dan tidak mengganggu akses",
      "Pot tanaman / area tanaman dicek, tidak ada sampah di dalam pot",
      "Tanaman disiram sesuai kebutuhan (terutama pagi/sore saat panas)",
      "Rumput kecil/liar yang terlihat mengganggu dicabut",
      "Area depan signage / pagar / jalan masuk dirapikan",
      "Genangan air, tanah becek, atau area licin wajib dilaporkan",
      "Tempat sampah outdoor dicek, tidak boleh penuh sampai meluber",
      "Rokok/puntung di area depan wajib dibersihkan",
      "Foto menampilkan area depan secara luas",
    ],
  },
  {
    code: "RTPL-PA05",
    title: "Taman, Tanaman & Rumput Kecil",
    category: "Maintenance",
    position_group: "PA",
    outlet_code: "KBU",
    standard_result:
      "Tanaman terlihat hidup dan rapi, tidak kering karena lupa disiram, pot bersih dari sampah, dan rumput liar kecil tidak dibiarkan tumbuh berantakan.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "15:00",
    target_time_end: "16:00",
    sort_order: 5,
    checklist: [
      "Semua tanaman area customer dicek satu per satu",
      "Tanaman disiram sesuai kondisi cuaca",
      "Daun kering yang jatuh dibersihkan",
      "Pot tanaman dibersihkan dari plastik, puntung rokok, tisu, atau sampah kecil",
      "Rumput kecil/liar di jalur customer dicabut",
      "Area sekitar tanaman disapu setelah disiram jika kotor",
      "Tanaman rusak/kering difoto dan dilaporkan",
      "Selang/alat siram dikembalikan ke tempatnya",
      "Tidak hanya foto tanaman tanpa benar-benar disiram",
      "Foto menunjukkan tanaman/area setelah rapi",
    ],
  },
  {
    code: "RTPL-PA06",
    title: "Sampah & Tempat Sampah Check",
    category: "Cleaning",
    position_group: "PA",
    outlet_code: "KBU",
    standard_result:
      "Tempat sampah tidak penuh, tidak bau, tidak meluber, plastik sampah rapi, dan area sekitar tempat sampah bersih.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "11:00",
    target_time_end: "12:00",
    sort_order: 6,
    checklist: [
      "Semua tempat sampah customer dicek",
      "Tempat sampah toilet dicek",
      "Tempat sampah outdoor dicek",
      "Sampah yang penuh segera dibuang ke titik pembuangan",
      "Plastik sampah diganti jika kotor, basah, atau bau",
      "Area sekitar tempat sampah disapu/pel jika ada tumpahan",
      "Tutup tempat sampah dibersihkan jika kotor",
      "Sampah kardus/botol/plastik besar dipisahkan jika mengganggu area",
      "Jika sampah belum bisa dibuang karena kendala, wajib tulis alasan jelas",
      "Foto menunjukkan tempat sampah setelah dibersihkan",
    ],
  },
  {
    code: "RTPL-PA07",
    title: "Mushola / Area Ibadah Check",
    category: "Cleaning",
    position_group: "PA",
    outlet_code: "KBU",
    standard_result:
      "Area ibadah bersih, tidak bau, alat ibadah rapi, lantai bersih, dan layak digunakan customer/staff.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "10:00",
    target_time_end: "11:00",
    sort_order: 7,
    checklist: [
      "Lantai mushola disapu dan dipel",
      "Sajadah/mukena/sarung dirapikan",
      "Area wudhu dicek, tidak licin dan tidak bau",
      "Sampah kecil dibersihkan",
      "Rak/perlengkapan ibadah dirapikan",
      "Kipas/lampu dicek menyala normal jika digunakan",
      "Bau lembap wajib dilaporkan",
      "Alat ibadah kotor wajib dipisahkan/lapor",
      "Tidak ada barang pribadi berantakan",
      "Foto area rapi setelah selesai",
    ],
  },
  {
    code: "RTPL-PA08",
    title: "Closing Public Area Customer",
    category: "Closing",
    position_group: "PA",
    outlet_code: "KBU",
    standard_result:
      "Area customer ditutup dalam kondisi bersih, sampah aman, toilet bersih, meja kursi rapi, lantai tidak meninggalkan kotoran untuk shift besok.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "21:00",
    target_time_end: "22:00",
    sort_order: 8,
    checklist: [
      "Semua meja customer dibersihkan",
      "Semua kursi dirapikan",
      "Lantai area makan disapu dan dipel titik kotor",
      "Kolong meja dicek ulang",
      "Toilet dicek ulang sebelum tutup",
      "Tempat sampah dicek dan dibuang jika penuh/bau",
      "Area depan dicek dari sampah malam (puntung rokok, plastik, tisu)",
      "Peralatan kebersihan dicuci/rendam/dirapikan sesuai tempat",
      "Kendala closing wajib ditulis jelas",
      "Foto menunjukkan kondisi akhir area setelah tutup",
    ],
  },
  // —— Kasir (1) ——
  {
    code: "RTPL-KSR01",
    title: "Opening & Closing Kas Harian",
    category: "Opening",
    position_group: "Kasir",
    standard_result:
      "Kas siap operasi: modal awal tercatat, peralatan kas berfungsi, area kas bersih, closing tercatat rapi.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "08:00",
    target_time_end: "09:00",
    sort_order: 1,
    checklist: [
      "Modal awal kas sudah diinput / dicatat",
      "Mesin kas / EDC / printer struk berfungsi",
      "Kertas struk dan alat tulis tersedia",
      "Area kas bersih dan rapi",
      "Float uang kecil sudah disiapkan",
      "Closing kas kemarin sudah direkonsiliasi",
      "Kendala kas (jika ada) sudah dilaporkan ke leader",
    ],
  },
  // —— Purchasing (1) ——
  {
    code: "RTPL-PRC01",
    title: "Follow Up PO & Supplier Harian",
    category: "Stock",
    position_group: "Purchasing",
    standard_result:
      "Semua PO aktif terpantau, supplier di-follow up, keterlambatan dan kekurangan barang tercatat jelas.",
    requires_photo: false,
    is_required_daily: true,
    target_time_start: "09:00",
    target_time_end: "11:00",
    sort_order: 1,
    checklist: [
      "Daftar PO aktif sudah dicek",
      "Supplier di-follow up untuk pengiriman hari ini",
      "Barang telat / partial delivery sudah dicatat",
      "Kebutuhan urgent outlet sudah dikomunikasikan",
      "Harga / qty discrepancy sudah dilaporkan",
      "Update status PO sudah diinput / dicatat",
    ],
  },
  // —— Gudang (1) ——
  {
    code: "RTPL-GDG01",
    title: "Cek Stok & Area Gudang",
    category: "Stock",
    position_group: "Gudang",
    standard_result:
      "Area gudang rapi, barang masuk/keluar tercatat, stok kritis teridentifikasi, suhu/kebersihan aman.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "08:30",
    target_time_end: "10:00",
    sort_order: 1,
    checklist: [
      "Barang masuk hari ini sudah dicek qty & kondisi",
      "Barang keluar sudah tercatat sesuai permintaan",
      "Rak / area penyimpanan rapi dan label jelas",
      "Stok kritis / habis sudah dilaporkan ke purchasing",
      "Area gudang bersih, tidak ada barang expired tergeletak",
      "Suhu penyimpanan (chiller/freezer) dicek jika relevan",
      "Foto kondisi area gudang diambil",
    ],
  },
  // —— Produksi FnB (1) ——
  {
    code: "RTPL-PFB01",
    title: "Produksi Harian FnB",
    category: "Production",
    position_group: "ProduksiFnB",
    standard_result:
      "Rencana produksi FnB jalan, BBP tercatat, kualitas produk sesuai standar, waste terkontrol.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "07:00",
    target_time_end: "09:00",
    sort_order: 1,
    checklist: [
      "Rencana produksi hari ini sudah dikonfirmasi",
      "BBP / bahan baku cukup untuk target produksi",
      "Proses produksi sesuai SOP & standar resep",
      "Hasil produksi sudah di-label / diidentifikasi",
      "Waste / reject tercatat dengan jelas",
      "Area produksi bersih sebelum dan sesudah proses",
      "Kendala produksi sudah dilaporkan ke leader",
    ],
  },
  // —— Produksi NF (1) ——
  {
    code: "RTPL-PNF01",
    title: "Produksi Harian NF",
    category: "Production",
    position_group: "ProduksiNF",
    standard_result:
      "Produksi produk NF sesuai jadwal, BBP tercatat, kualitas & packaging sesuai standar NF.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "07:00",
    target_time_end: "09:00",
    sort_order: 1,
    checklist: [
      "Target produksi NF hari ini sudah dikonfirmasi",
      "BBP produk NF cukup dan dalam kondisi baik",
      "Proses produksi mengikuti SOP produk NF",
      "Packaging / labeling sesuai standar brand NF",
      "Sample quality check sudah dilakukan",
      "Waste / reject produk NF tercatat",
      "Area kerja bersih dan alat produksi dirapikan",
    ],
  },
  // —— Advertising (1) ——
  {
    code: "RTPL-ADV01",
    title: "Konten & Campaign Harian",
    category: "General",
    position_group: "Advertising",
    standard_result:
      "Jadwal konten terpantau, campaign aktif dimonitor, material siap upload / tayang sesuai plan.",
    requires_photo: false,
    is_required_daily: true,
    target_time_start: "09:00",
    target_time_end: "12:00",
    sort_order: 1,
    checklist: [
      "Jadwal konten hari ini sudah dicek",
      "Material iklan / konten sudah disiapkan atau dipublish",
      "Campaign aktif dimonitor (reach, engagement, error)",
      "Request konten dari outlet sudah ditindaklanjuti",
      "Asset / copy sudah disimpan rapi di folder standar",
      "Kendala publikasi sudah dilaporkan ke leader",
    ],
  },
  // —— Admin MP (1) ——
  {
    code: "RTPL-AMP01",
    title: "Admin MP Harian",
    category: "General",
    position_group: "AdminMP",
    standard_result:
      "Dokumen admin MP terupdate, permintaan internal ditindaklanjuti, arsip rapi.",
    requires_photo: false,
    is_required_daily: true,
    target_time_start: "09:00",
    target_time_end: "11:00",
    sort_order: 1,
    checklist: [
      "Inbox / permintaan admin MP sudah dicek",
      "Dokumen wajib hari ini sudah diproses",
      "Data master / input admin sudah diupdate",
      "Arsip dokumen rapi dan mudah dilacak",
      "Follow up ke divisi terkait sudah dilakukan",
      "Kendala admin sudah dilaporkan ke leader",
    ],
  },
  // —— CS NF (1) ——
  {
    code: "RTPL-CSN01",
    title: "Layanan CS NF Harian",
    category: "General",
    position_group: "CSNF",
    standard_result:
      "Ticket / chat customer ditangani, SLA respons terpenuhi, eskalasi kendala sudah jelas.",
    requires_photo: false,
    is_required_daily: true,
    target_time_start: "08:00",
    target_time_end: "17:00",
    sort_order: 1,
    checklist: [
      "Inbox / ticket customer sudah dicek pagi hari",
      "Pertanyaan / komplain ditanggapi sesuai SLA",
      "Case sulit sudah dieskalasi ke leader / terkait",
      "Status ticket sudah diupdate di sistem / catatan",
      "Follow up customer pending sudah dilakukan",
      "Rekap kendala harian sudah dicatat",
    ],
  },
  // —— Finance (1) ——
  {
    code: "RTPL-FIN01",
    title: "Rekonsiliasi Keuangan Harian",
    category: "General",
    position_group: "Finance",
    standard_result:
      "Transaksi harian tercatat, rekonsiliasi kas/bank dicek, selisih dan kendala sudah dilaporkan.",
    requires_photo: false,
    is_required_daily: true,
    target_time_start: "10:00",
    target_time_end: "15:00",
    sort_order: 1,
    checklist: [
      "Laporan transaksi harian sudah diterima dari outlet",
      "Rekonsiliasi kas / transfer sudah dicek",
      "Selisih (jika ada) sudah dianalisa & dicatat",
      "Tagihan / invoice pending sudah di-follow up",
      "Dokumen keuangan hari ini sudah diarsipkan",
      "Kendala keuangan sudah dilaporkan ke leader / owner",
    ],
  },
  // —— Design (1) ——
  {
    code: "RTPL-DSN01",
    title: "Antrian Design Harian",
    category: "General",
    position_group: "Design",
    standard_result:
      "Antrian design terpantau, request diselesaikan sesuai prioritas, file final rapi & terkirim.",
    requires_photo: false,
    is_required_daily: true,
    target_time_start: "09:00",
    target_time_end: "17:00",
    sort_order: 1,
    checklist: [
      "Antrian request design hari ini sudah dicek",
      "Prioritas pekerjaan sudah dikonfirmasi dengan leader",
      "Draft / revisi design sudah dikerjakan sesuai deadline",
      "File final disimpan di folder standar dengan naming benar",
      "Asset sudah dikirim / diupload ke pihak terkait",
      "Kendala deadline / brief kurang jelas sudah dilaporkan",
    ],
  },
  // —— Outlet Crew — Samtaro Express (mini outlet, 1 orang merangkap) ——
  {
    code: "RTPL-OC01",
    title: "Opening Outlet Mini",
    category: "Opening",
    position_group: "OutletCrew",
    outlet_code: "Samtaro Express",
    standard_result:
      "Outlet siap beroperasi: area bersih, kasir siap, alat menyala, dan perlengkapan lengkap.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "07:00",
    target_time_end: "08:00",
    sort_order: 1,
    checklist: [
      "Buka outlet dan pastikan akses aman",
      "Nyalakan lampu dan peralatan operasional",
      "Cek koneksi internet",
      "Cek QRIS atau metode pembayaran",
      "Siapkan kas awal sesuai nominal",
      "Bersihkan meja kasir",
      "Rapikan meja dan kursi pelanggan",
      "Cek tisu, plastik, sedotan, dan perlengkapan pelayanan",
    ],
  },
  {
    code: "RTPL-OC02",
    title: "Cek Kualitas Minuman Awal",
    category: "Quality Control",
    position_group: "OutletCrew",
    outlet_code: "Samtaro Express",
    standard_result:
      "Cicipi produk minuman awal untuk memastikan rasa, aroma, suhu, dan bahan masih layak.",
    requires_photo: false,
    is_required_daily: true,
    target_time_start: "08:00",
    target_time_end: "09:00",
    sort_order: 2,
    checklist: [
      "Cicipi kopi susu aren atau produk kopi utama",
      "Cek rasa tidak terlalu manis, pahit, atau hambar",
      "Cek aroma produk normal",
      "Cek susu masih layak dan tidak asam",
      "Cek gula aren atau bahan cair tidak berubah rasa",
      "Cek es batu bersih dan tidak berbau",
      "Catat produk yang rasanya tidak sesuai",
    ],
  },
  {
    code: "RTPL-OC03",
    title: "Cek Stok Awal",
    category: "Stock",
    position_group: "OutletCrew",
    outlet_code: "Samtaro Express",
    standard_result:
      "Pastikan bahan baku dan kemasan cukup untuk penjualan hari ini.",
    requires_photo: false,
    is_required_daily: true,
    target_time_start: "08:00",
    target_time_end: "09:00",
    sort_order: 3,
    checklist: [
      "Cek stok cup",
      "Cek tutup cup",
      "Cek sedotan",
      "Cek plastik takeaway",
      "Cek susu",
      "Cek kopi",
      "Cek gula aren",
      "Cek bahan minuman lainnya",
      "Cek air galon",
      "Cek es batu",
      "Catat barang yang menipis atau habis",
    ],
  },
  {
    code: "RTPL-OC04",
    title: "Kebersihan Area Outlet",
    category: "Cleaning",
    position_group: "OutletCrew",
    outlet_code: "Samtaro Express",
    standard_result:
      "Area pelayanan, kasir, meja pelanggan, dan sekitar outlet tetap bersih dan rapi.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "10:00",
    target_time_end: "20:00",
    sort_order: 4,
    checklist: [
      "Lantai bersih",
      "Meja pelanggan bersih",
      "Kursi rapi",
      "Area kasir bersih",
      "Area pembuatan minuman bersih",
      "Peralatan tidak menumpuk kotor",
      "Tempat sampah tidak penuh",
      "Wastafel atau area cuci bersih",
      "Area depan outlet bersih",
    ],
  },
  {
    code: "RTPL-OC05",
    title: "Monitoring Operasional",
    category: "General",
    position_group: "OutletCrew",
    outlet_code: "Samtaro Express",
    standard_result:
      "Pastikan pelayanan, produk, peralatan, dan kondisi outlet berjalan normal. Tidak menggantikan Lapor Kendala Operasional.",
    requires_photo: false,
    is_required_daily: true,
    target_time_start: "09:00",
    target_time_end: "21:00",
    sort_order: 5,
    checklist: [
      "Pelayanan pelanggan berjalan baik",
      "Pesanan tercatat dengan benar",
      "Produk dibuat sesuai SOP",
      "Tidak ada alat yang rusak",
      "Tidak ada kebocoran atau masalah listrik",
      "Tidak ada komplain pelanggan yang belum ditangani",
      "Catat kendala operasional jika ada",
    ],
  },
  {
    code: "RTPL-OC06",
    title: "Restock Operasional",
    category: "Stock",
    position_group: "OutletCrew",
    outlet_code: "Samtaro Express",
    standard_result:
      "Tambahkan stok operasional sebelum bahan atau kemasan habis.",
    requires_photo: false,
    is_required_daily: false,
    kind: "special_task",
    target_time_start: "10:00",
    target_time_end: "20:00",
    sort_order: 6,
    checklist: [
      "Tambah cup",
      "Tambah tutup cup",
      "Tambah sedotan",
      "Tambah susu",
      "Tambah gula aren",
      "Tambah kopi",
      "Tambah es batu",
      "Catat barang yang tidak tersedia untuk direstock",
    ],
  },
  {
    code: "RTPL-OC07",
    title: "Closing dan Kebersihan Akhir",
    category: "Closing",
    position_group: "OutletCrew",
    outlet_code: "Samtaro Express",
    standard_result:
      "Outlet ditutup dalam keadaan bersih, aman, dan seluruh peralatan sudah diperiksa.",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "21:00",
    target_time_end: "22:00",
    sort_order: 7,
    checklist: [
      "Bersihkan alat pembuatan minuman",
      "Bersihkan meja kasir",
      "Bersihkan meja pelanggan",
      "Sapu dan pel lantai",
      "Buang sampah",
      "Bersihkan area cuci",
      "Rapikan bahan dan kemasan",
      "Simpan bahan sesuai tempatnya",
      "Matikan peralatan yang tidak digunakan",
      "Matikan lampu sesuai kebutuhan",
      "Pastikan pintu atau akses outlet terkunci",
    ],
  },
  {
    code: "RTPL-OC08",
    title: "Stok Opname Akhir",
    category: "Stock",
    position_group: "OutletCrew",
    outlet_code: "Samtaro Express",
    standard_result:
      "Hitung sisa bahan dan kemasan untuk mengetahui pemakaian serta kebutuhan hari berikutnya. Catat hasil opname di kolom catatan (tanpa angka default otomatis).",
    requires_photo: true,
    is_required_daily: true,
    target_time_start: "21:30",
    target_time_end: "22:30",
    sort_order: 8,
    checklist: [
      "Hitung sisa cup",
      "Hitung sisa tutup cup",
      "Hitung sisa sedotan",
      "Hitung sisa plastik",
      "Hitung sisa susu",
      "Hitung sisa kopi",
      "Hitung sisa gula aren",
      "Hitung sisa bahan minuman lainnya",
      "Catat barang habis",
      "Catat barang yang perlu diminta ke gudang atau purchasing",
    ],
  },
  {
    code: "RTPL-OC09",
    title: "Rekap Kas dan Penjualan",
    category: "Kasir",
    position_group: "OutletCrew",
    outlet_code: "Samtaro Express",
    standard_result:
      "Cocokkan transaksi, uang tunai, QRIS, dan catatan penjualan sebelum laporan akhir.",
    requires_photo: false,
    is_required_daily: true,
    target_time_start: "21:30",
    target_time_end: "22:30",
    sort_order: 9,
    checklist: [
      "Cek total transaksi POS",
      "Cek total pembayaran tunai",
      "Cek total pembayaran QRIS",
      "Cocokkan uang kas fisik",
      "Catat selisih jika ada",
      "Pastikan transaksi tidak ada yang tertinggal",
      "Lakukan setoran atau simpan kas sesuai SOP",
    ],
  },
  {
    code: "RTPL-OC10",
    title: "Catatan Harian Outlet",
    category: "General",
    position_group: "OutletCrew",
    outlet_code: "Samtaro Express",
    standard_result:
      "Isi ringkasan kondisi outlet agar leader mengetahui kebutuhan dan masalah hari ini. Boleh tulis \"Tidak ada kendala\" jika kondisi normal.",
    requires_photo: false,
    requires_note: true,
    is_required_daily: true,
    target_time_start: "22:00",
    target_time_end: "22:30",
    sort_order: 10,
    checklist: [
      "Catat produk yang habis",
      "Catat stok yang menipis",
      "Catat barang rusak",
      "Catat alat bermasalah",
      "Catat komplain pelanggan",
      "Catat permintaan stok",
      "Catat kebutuhan perbaikan",
      "Catat informasi penting untuk shift atau hari berikutnya",
    ],
  },
  // —— Quick kendala (semua posisi) ——
  {
    code: "RTPL-K01",
    title: "Lapor Kendala Operasional",
    category: "Kendala",
    position_group: null,
    outlet_code: null,
    standard_result: "Kendala tercatat jelas agar leader bisa follow up.",
    requires_photo: false,
    is_required_daily: false,
    kind: "issue_quick",
    sort_order: 99,
    checklist: [
      "Jenis kendala sudah dipilih di status kondisi",
      "Lokasi / area kendala sudah jelas di catatan",
      "Foto diambil jika relevan (opsional)",
    ],
  },
];
